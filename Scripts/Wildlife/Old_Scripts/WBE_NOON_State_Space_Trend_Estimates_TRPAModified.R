...
#  correct.
#
#  Create the following data sheet in Excel, name it osprey and save it 
...
# Read in data set
my_data <- read.csv("ba_winter.csv", header=T,sep=",")
head(my_data)
...
sink(file = "ba_winter_results.txt", append = FALSE, 
     type = "output", split = T)    # will create a file with your results in wd
...
# calcualte the 90% confidence interval 
mu_hi90.reml=mu.reml+1.645*sqrt(Var_mu.reml);  #  90% CI for mu
mu_lo90.reml=mu.reml-1.645*sqrt(Var_mu.reml);
...
bmp(file = "wbe_graph.bmp") # Will save the plot in wd. 
