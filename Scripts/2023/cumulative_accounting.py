...
def summarize_landcap_by_parcel():
    # get paths
    ...
    parcels = arcpy.management.MakeFeatureLayer(str(parcelPath), 'parcels', '"YEAR" = 2023')
    landcap = arcpy.management.MakeFeatureLayer(str(landcapPath), 'landcap')
    # summarize within
    arcpy.analysis.SummarizeWithin(
        in_polygons=parcels,
        in_sum_features=landcap,
        out_feature_class=r"C:\GIS\Scratch.gdb\Summarize_ParcelLandCapabalities",
...
def get_summary():
    ...
    path = r"C:\GIS\Scratch.gdb\Summarize_ParcelLandCapabalities"
    # query 2022 rows
    ...
    df = df.groupby(['Category', 'DevelopmentType']).sum().reset_index()
    return df, sdf
